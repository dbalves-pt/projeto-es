...
public class PainelEquipas extends JPanel {
    ...
    private JPanel criarColunaMeio() {
        ...
        // ── BOTÃO ELIMINAR EQUIPA NO FUNDO ──
        JButton btnEliminarEq = new JButton("Eliminar equipa...");
        btnEliminarEq.setContentAreaFilled(false);
        btnEliminarEq.setBorder(new EmptyBorder(10, 0, 5, 0));
        btnEliminarEq.setAlignmentX(Component.LEFT_ALIGNMENT);
        btnEliminarEq.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        btnEliminarEq.addActionListener(e -> {
            if (equipaSelecionadaAtual != null) {
                int confirmacao = JOptionPane.showConfirmDialog(this, "Deseja eliminar a equipa " + equipaSelecionadaAtual.getNome() + "?", "Confirmar", JOptionPane.YES_NO_OPTION);
                if (confirmacao == JOptionPane.YES_OPTION) {
                    try {
                        equipaControlador.eliminarEquipa(equipaSelecionadaAtual);
                        equipaSelecionadaAtual = null;
                        atualizarListasAposEdicao();
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  COLUNA 3: Visão Geral dos Grupos (A preencher no futuro)
    // ══════════════════════════════════════════════════════════════════════════
    // ══════════════════════════════════════════════════════════════════════════
    ...
    private JPanel criarColunaDireita() {
        ...
        lblGrupoA.setFont(new Font("SansSerif", Font.BOLD, 14));
        lblGrupoA.setAlignmentX(Component.LEFT_ALIGNMENT);
        painel.add(lblGrupoA);
        painel.add(Box.createVerticalStrut(5));
        ...
        painel.add(criarTabelaGrupo(dadosA, colunas));
        painel.add(Box.createVerticalStrut(15));

        // --- GRUPO B ---
        JLabel lblGrupoB = new JLabel("Grupo B");
        lblGrupoB.setFont(new Font("SansSerif", Font.BOLD, 14));
        lblGrupoB.setAlignmentX(Component.LEFT_ALIGNMENT);
        painel.add(lblGrupoB);
        painel.add(Box.createVerticalStrut(5));
        ...
        painel.add(criarTabelaGrupo(dadosB, colunas));

        // Empurra tudo para cima para não ficar espalhado pelo cartão
        painel.add(Box.createVerticalGlue());
